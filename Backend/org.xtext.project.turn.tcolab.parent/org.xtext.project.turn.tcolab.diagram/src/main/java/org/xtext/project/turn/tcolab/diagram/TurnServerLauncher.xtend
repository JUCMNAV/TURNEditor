...
class TurnServerLauncher extends ServerLauncher {
		...
		new TurnIdeSetup {
			override createInjector() {
				Guice.createInjector(
					Modules2.mixin(new TurnRuntimeModule as Module, new TurnIdeModule, new TurnDiagramModule))
			}
		}.createInjectorAndDoEMFRegistration()

		// Launch the server
		launch(ServerLauncher.name, args, Modules2.mixin(new ServerModule, [
			bind(ServerLauncher).to(TurnServerLauncher)
