...
class TurnDiagramGenerator implements IDiagramGenerator {
	...
	override generate(Context context) {
		...
		val elle = context.state.getSelectedElements();
		val elle2 = context.state.getExpandedElements();
		LOG.info("Inside Generate Diagram ")
...
//						}
	...
	def SModelRoot generateDiagram(Actor actor, CancelIndicator cancelIndicator) {
