...
export class ContextMenuCommands {
    ...
    registerCommand(id: string, callback: (...args: any[]) => any, thisArg?: any): Disposable {
        ...
        const removeCommand = this.registry.registerCommand({ id: id }, {
            execute: () => {
                ...
                if (this.isTURNEditor(currentEditor)) {
                    execute(currentEditor.editor.document.uri)
                }
            },
            isVisible: () => this.isTURNEditor(this.editorManager.currentEditor)
        });
        const removeMenu = this.menuRegistry.registerMenuAction(EDITOR_CONTEXT_MENU.concat("2_turn"), {
            commandId: id,
            label: id
        });
        return {
    ...
    private isTURNEditor(widget: EditorWidget | undefined): widget is EditorWidget {
        if (widget)
            return widget.editor.document.languageId === 'turn';
        else
            return false;
    }
}